...
@torch.no_grad()
def reMissOpt(cfg, instruct, target, prompter, target_llm, record_stats=False, rank=0):
    ...
    # Initial response from the target model
    ...
    if baseline_ids.shape[1] != target_ids.shape[1]:
        if cfg.verbose and rank == 0:
            tqdm.write(f" Length mismatch detected: baseline {baseline_ids.shape[1]} vs target {target_ids.shape[1]}")
        
        max_len = max(baseline_ids.shape[1], target_ids.shape[1])
        
        # pad baseline_ids to match max length
        if baseline_ids.shape[1] < max_len:
            pad_size = max_len - baseline_ids.shape[1]
            baseline_ids = torch.nn.functional.pad(
                baseline_ids, 
                (0, pad_size), 
                value=baseline.tokenizer.pad_token_id
            )
            if cfg.verbose and rank == 0:
                tqdm.write(f" Padded baseline from {baseline.ids.shape[1]} to {baseline_ids.shape[1]}")
        
        # pad target_ids to match max length  
        if target_ids.shape[1] < max_len:
            pad_size = max_len - target_ids.shape[1]
            target_ids = torch.nn.functional.pad(
                target_ids,
                (0, pad_size),
                value=target.tokenizer.pad_token_id
            )
            if cfg.verbose and rank == 0:
                tqdm.write(f" Padded target from {target.ids.shape[1]} to {target_ids.shape[1]}")
